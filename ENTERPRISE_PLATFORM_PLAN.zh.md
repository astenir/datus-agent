...
| 企业 Agent 管理 | `datus_enterprise/api/agent_routes.py`、`datus_enterprise/agent_registry.py` | 新企业 API 使用 `/api/v1/agents` 返回当前用户可用 agent 目录，使用 `/api/v1/admin/agents*` 管理企业 custom agent metadata、状态和 ACL；运行时仍通过 `/api/v1/chat/stream` 的 `subagent_id` dispatch，执行前校验 agent 状态、ACL、node_class 对应模块权限，并把企业 agent 定义只物化到请求级 `AgentConfig` clone。旧 `/api/v1/agent/*` 继续作为 legacy route 在企业模式禁用。 |
...
| `ArtifactAclStore` | 查询 artifact ACL 和可见列表 | manifest / local-compatible | Postgres artifact metadata |
...
- 已新增 PostgreSQL-backed enterprise metadata stores，可通过 `enterprise.*.class` loader 配置启用，覆盖 user、role、datasource grant、enterprise agent、session owner、artifact ACL、audit、quota 和 secret reference metadata。生产多 worker/HA 建议使用 `conf/agent.enterprise.pg.yml.example`，SQLite MVP 配置仅适合本地单节点或试点。
- 已新增可选 `enterprise.session_body_store`，当前实现为 `datus_enterprise.postgres_session_store:PgSessionBodyStore`。它迁移的是聊天正文/状态持久化路径：messages/items、message structure、turn usage、running turn usage 和 system-prompt snapshot。`session_owners` 仍只负责企业 session owner/index metadata，不保存正文。
- 本次 PG 化仍不迁移 LanceDB/RAG vector backend、`subject/semantic_models`、`subject/sql_summaries`、报表/仪表盘 bundle/export 文件、artifact 二进制文件或业务 datasource 查询配置。
...
| 企业 custom agent 管理 | `module.admin.agents` |
...
- `conf/agent.enterprise.pg.yml.example`：通过 `datus_enterprise.postgres_stores:*` provider 将 enterprise user、role、datasource grant、enterprise agent、session owner、artifact ACL、audit、quota 和 secret reference metadata 放入 PostgreSQL，并可通过 `datus_enterprise.postgres_session_store:PgSessionBodyStore` 将聊天正文/状态放入 PostgreSQL，适合作为生产多 worker/HA 的基础配置。

这两套配置都不改变业务 datasource 查询逻辑，也不改变 RAG/vector 或项目文件系统内容的存储位置。只有显式配置 `enterprise.session_body_store` 时，新打开的 `SessionManager` 才会把 chat session 正文历史写入 PostgreSQL；未配置时继续使用本地 `AdvancedSQLiteSession` SQLite `.db` 文件。
artifact ACL 和 artifact metadata 属于企业平台 metadata；报表/仪表盘实际 bundle、静态 HTML、导出文件等二进制或静态产物仍留在当前文件系统路径，后续如需迁移应进入对象存储切片，而不是写入 artifact ACL metadata 表。
secret reference store 只保存外部密钥引用 metadata，不保存 secret 明文；store 内部记录会包含原始 reference 以便后续执行期解析，任何 admin API 响应、审计摘要和日志都必须先转换为 `ref_hint`，不得直接回显或记录原始 reference。

### PG session body backend

`enterprise.session_body_store` 是聊天正文/状态 backend，不是 session owner metadata store。当前 PG 实现覆盖：

- `AdvancedSQLiteSession.add_items()` / `get_items()` / `pop_item()` / `clear_session()` 兼容接口。
- `message_structure` 对等数据，用于按 branch/sequence 恢复上下文。
- `turn_usage` 对等数据，用于会话 token usage 汇总和 compact 前后估算。
- `running_turn_usage`，用于 turn 进行中时的 usage 快照。
- system-prompt snapshot，替代本地 `{session_id}.sysprompt.json` sidecar。
- `SessionManager.list_sessions()` / `get_session_info()` / `get_session_messages()` / `delete_session()` / `copy_session()` 的 API 读取路径。

当前边界：

- 不自动扫描或迁移历史 SQLite `.db`。开启 PG body backend 后只影响新 `SessionManager` 通过该 backend 打开的会话；历史本地会话按原 SQLite 文件保留。
- 回滚方式是移除 `enterprise.session_body_store` 配置并重启服务；回滚后新请求回到本地 SQLite backend，但 PG 中已写入的会话正文不会自动回写为 `.db` 文件。
- `SessionOwnerStore` 与 `SessionBodyStore` 不一致时，API 授权仍以 `SessionOwnerStore` 为入口 fail closed；body store 只保存正文，不授予访问权。启用 PG body backend 后，普通用户 list/history/delete/resume/feedback 等路径不得因为正文表中存在 session 而自动修复或创建 owner 记录；用户可见 session list 也必须按 owner index 过滤。
- 表结构只做 `CREATE TABLE IF NOT EXISTS` / `CREATE INDEX IF NOT EXISTS`，不做破坏性 DDL、隐式历史迁移、字段回填或 schema version 管理。
- 真实 PostgreSQL integration 测试使用 `DATUS_ENTERPRISE_PG_DSN` gate，并使用唯一前缀清理测试数据。
- 备份恢复必须同时考虑 enterprise metadata PG 和 session body PG；如果二者共用同一个 PostgreSQL 实例，可以纳入同一备份策略，但恢复校验必须分别确认 owner/index metadata 与正文表一致。
- 滚动发布时，未启用 `session_body_store` 的旧 pod 仍写本地 SQLite，启用后的新 pod 写 PG；生产切换应按环境级开关整体发布，避免同一用户会话在两种 backend 间来回写入。需要蓝绿或灰度时，应限制到新 session 或明确 sticky routing。
- 不迁移 RAG/vector、`subject/semantic_models`、`subject/sql_summaries`、artifact bundle/export、业务 datasource 数据。

### PG metadata schema 生命周期

当前 `datus_enterprise.postgres_stores` 的 PostgreSQL schema 仍由模块内 `_SCHEMA_SQL` 在首次使用时执行 `CREATE TABLE IF NOT EXISTS` 和 `CREATE INDEX IF NOT EXISTS` 管理。这个机制只作为开发、试点和首次 bootstrap 的最小自动建表能力，不是完整生产迁移系统。

当前边界：

- 只做幂等建表和建索引，不做 `ALTER TABLE`、字段回填、数据修复、schema version 记录、降级回滚或在线迁移编排。
- 不删除旧表、旧列或旧索引；不在应用启动时执行破坏性 DDL。
- 新增 metadata 字段时，必须单独评估老版本代码、滚动发布、备份恢复和已有数据兼容性；不能只依赖 `CREATE TABLE IF NOT EXISTS`。
- 每个 PG metadata/body store 当前各自持有一个 asyncpg pool；默认 `max_size=2`，`conf/agent.enterprise.pg.yml.example` 显式写出 `min_size=1` / `max_size=2`。生产连接数预算至少按 `store 数量 * max_size * API 进程数` 计算，并叠加业务 datasource、运维连接和数据库 `max_connections` 余量。当前 PG overlay 中 9 个 metadata store + 1 个 session body store 在 `max_size=2` 时，每个 API 进程最多占用 20 个 enterprise PG 连接。
- 真实 PostgreSQL integration 测试必须 gated，例如使用 `DATUS_ENTERPRISE_PG_DSN`，并使用唯一前缀清理自己写入的数据，不能破坏已有库内容。
- 生产部署必须把 enterprise metadata PostgreSQL 纳入备份恢复计划；启用 `enterprise.session_body_store` 时，聊天正文/状态表也必须纳入备份恢复计划。metadata 备份覆盖 users、roles、datasource grants、enterprise agents、session owner index、artifact ACL、audit、quota 和 secret reference metadata；session body 备份覆盖 messages/items、message structure、turn usage、running usage 和 prompt snapshot；二者都不覆盖 RAG/vector、项目 `subject/` 文件、报表/仪表盘 bundle 或导出文件。

后续生产迁移切片建议引入独立 migration runner，例如 Alembic 或项目自有迁移命令，至少包含 schema version 表、前向迁移、回滚策略、dry-run、迁移锁、滚动发布兼容说明和备份恢复演练。迁移系统落地前，PG metadata schema 变更应保持小步、向后兼容，并在 PR/提交说明中明确是否需要人工 DDL。
...
CREATE TABLE enterprise_artifact_acls (
...
CREATE INDEX idx_enterprise_artifact_acls_type_updated ON enterprise_artifact_acls (artifact_type, updated_at);
...
- 企业 PG 配置样例已放在 `conf/agent.enterprise.pg.yml.example`，覆盖 RBAC/user/role/datasource grant/enterprise agent/session owner/artifact ACL/audit/quota/secret reference metadata，并包含可选的 `PgSessionBodyStore` 聊天正文/状态 backend。它是生产多 worker/HA 的推荐起点；SQLite MVP 配置继续作为本地单节点 fallback。`datus_enterprise.postgres_stores.PgEnterpriseAgentStore` 只保存企业 custom agent 定义、状态和 ACL metadata；chat 执行时仍只把可用 agent 物化到请求级 `AgentConfig` clone。`datus_enterprise.postgres_stores.PgArtifactAclStore` 只保存企业平台 artifact ACL metadata，不迁移报表/仪表盘 bundle、静态 HTML 或导出文件；`PgSessionBodyStore` 不迁移历史 SQLite `.db`，只在显式启用后承接新 session body 写入。
...
- `DATUS_PLATFORM_STATUS=readonly` 时 SQL execute、semantic model save 和 admin mutation 在业务服务/metadata store 执行前返回 `PLATFORM_STATUS_FORBIDDEN` 并写审计；状态 gate 拒绝路径不得先初始化 `DatusService`，mutation route 应用 route-level dependency 或等价顺序测试证明 gate 先于 `ServiceDep`。
- table detail 和 semantic model route 的 datasource/table grant 测试必须覆盖至少一种 catalog-capable dialect，例如 StarRocks `catalog.database.table`，避免 dialect-blind parser 错拒或错放 catalog/database/schema scope。
- `get_request_app_context()` 应有直接单元测试覆盖 authenticate + refresh + cache、已缓存但未 enterprise-ready 的上下文刷新、缺少用户或禁用用户 fail closed；不要只依赖 route override 间接覆盖。
