...
- 已新增 `EnterpriseQuotaStore` 协议，以及本地兼容的进程内 `InMemoryEnterpriseQuotaStore`；企业模式下可通过 `enterprise.quota_store.class` 替换为生产 quota/usage metadata store。当前已将直接 SQL 执行入口接入 `sql.execute` 配额消耗，缺少 quota store、quota 检查失败或超额都会在执行前返回稳定错误并写入 audit；chat token、模型 token、dashboard query 和并发类配额仍按后续切片接入。
