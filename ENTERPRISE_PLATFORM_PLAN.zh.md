...
- `conf/agent.enterprise.pg.yml.example`：通过 `datus_enterprise.postgres_stores:*` provider 将 enterprise user、role、datasource grant、session owner、artifact ACL、audit、quota 和 secret reference metadata 放入 PostgreSQL，适合作为生产多 worker/HA 的基础配置。
...
- 每个 PG metadata store 当前各自持有一个 asyncpg pool；默认 `max_size=2`，`conf/agent.enterprise.pg.yml.example` 显式写出 `min_size=1` / `max_size=2`。生产连接数预算至少按 `store 数量 * max_size * API 进程数` 计算，并叠加业务 datasource、运维连接和数据库 `max_connections` 余量。
- 真实 PostgreSQL integration 测试必须 gated，例如使用 `DATUS_ENTERPRISE_PG_DSN`，并使用唯一前缀清理自己写入的数据，不能破坏已有库内容。
- 生产部署必须把 enterprise metadata PostgreSQL 纳入备份恢复计划；该备份只覆盖 users、roles、datasource grants、session owner index、artifact ACL、audit、quota 和 secret reference metadata，不覆盖 chat 正文历史、RAG/vector、项目 `subject/` 文件、报表/仪表盘 bundle 或导出文件。
...
- 企业 PG metadata 配置样例已放在 `conf/agent.enterprise.pg.yml.example`，覆盖 RBAC/user/role/datasource grant/session owner/artifact ACL/audit/quota/secret reference。它是生产多 worker/HA 的推荐起点；SQLite MVP 配置继续作为本地单节点 fallback。`datus_enterprise.postgres_stores.PgArtifactAclStore` 只保存企业平台 artifact ACL metadata，不迁移报表/仪表盘 bundle、静态 HTML 或导出文件。
